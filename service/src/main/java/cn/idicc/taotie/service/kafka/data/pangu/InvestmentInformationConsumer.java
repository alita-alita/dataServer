...
import cn.idicc.taotie.service.services.data.pangu.InvestmentInformationService;
 ...
 * @Author: WangZi
 * @Date: 2023/3/29
 * @Description: 处理导入招商资讯消息消费者
 * @version: 1.0
 */
@Slf4j
@Component
@DependsOn(value = {"namespaceProperties"})
public class InvestmentInformationConsumer {

    @Autowired
    private InvestmentInformationService investmentInformationService;
    ...
    @KafkaListener(topics = {"${namespace.kafka.prefix}" + TopicConstant.UPLOAD_INVESTMENT_INFORMATION_TOPIC})
    public void consumerUploadInvestmentInformation(ConsumerRecord<String, String> record, Acknowledgment ack) {
        ...
        try {
            if (Objects.nonNull(uploadFileRecordDO)) {
                ...
                if (content != null) {
                    List<InvestmentInformationUploadDTO> list = PanGuExcelUtil.readExcel(content,
                            excelProperties.getHeaderRowIndex(),
                            excelProperties.getStartRowIndex(),
                            excelProperties.getEndRowIndex(),
                            InvestmentInformationUploadDTO.class);
                    Set<InvestmentInformationUploadDTO> errorList = investmentInformationService.uploadBatchSave(list, id);
            ...
            log.error("消费导入招商资讯任务失败，任务id为{}", uploadFileRecordDO.getId(), e);
